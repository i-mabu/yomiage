...
client.on('interactionCreate', async (interaction) => {
    ...
    if (commandName === 'join') {
        ...
        if (!voiceChannel) {
            return interaction.reply({ content: '先にボイスチャンネルに入ってください！', ephemeral: true });
        }
        ...
        return interaction.reply('接続しました。このチャンネルの文字を読み上げます！');
    }
    ...
    if (commandName === 'leave') {
        ...
        return interaction.reply({ content: 'ボットはVCに参加していません。', ephemeral: true });
    }
});
    ...
    if (state && message.channelId === state.textChannelId) {
        ...
        try {
            const queryResponse = await axios.post(
                `${process.env.VOICEVOX_URL}/audio_query?text=${encodeURIComponent(message.content)}&speaker=3`
            );
        ...
        if (humanMembers.size === 0) {
            ...
            setTimeout(() => {
                ...
                if (currentConnection) {
                    ...
                    const freshChannel = oldState.guild.channels.cache.get(state.voiceChannelId);
                    if (freshChannel) {
                        const freshHumans = freshChannel.members.filter(m => !m.user.bot);
                        if (freshHumans.size === 0) {
                            currentConnection.destroy();
                            connectionMap.delete(guildId);
                            console.log(`[${oldState.guild.name}] 人がいなくなったため自動切断しました。`);
                        }
                    }
                }
            }, 3000); 
